---
printing: "1.01b"
printing_book: "Draw Steel: Heroes"
class: conduit
level: "1"
name: Prayer of Destruction
scc: mcdm.heroes.v1/feature.conduit.level-1/prayer-of-destruction
type: feature
---

# Prayer of Destruction

---

<section class="sc-trait sc-trait--crest sc-trait--lead" data-action="trait">
<header class="sc-head"><div class="sc-head__stack"><span class="sc-crest sc-trait__crest"><span class="sc-trait__glyph">*</span></span>
<div class="sc-head__col sc-head__col--left"><div class="sc-head__slot sc-head__left-eyebrow sc-head__slot--line">Feature</div><h3 class="sc-head__slot sc-head__left-primary sc-head__slot--line">Prayer of Destruction</h3><div class="sc-head__slot sc-head__left-deck sc-head__slot--line">Conduit</div></div></div><div class="sc-head__rail sc-head__col--right"><div class="sc-head__slot sc-head__right-eyebrow sc-head__slot--chip">Level 1</div></div></header>
<div class="sc-trait__body">
<p>Your god infuses wrath within your being. You gain a +1 <a href="../../../../rule/dice/bonuses-and-penalties/">bonus</a> to <a href="../../../../rule/damage/rolled-damage/">rolled damage</a> with magic abilities.</p>
</div>
</section>


<template class="sc-src" data-fmt="md" data-src="Your god infuses wrath within your being. You gain a +1 [bonus](../../../rule/dice/bonuses-and-penalties.md) to [rolled damage](../../../rule/damage/rolled-damage.md) with magic abilities."></template>
