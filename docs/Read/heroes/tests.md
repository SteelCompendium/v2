...
<div class="sc-cheyebrow">Draw Steel: Heroes · Chapter 9</div>

# Tests {.sc-chtitle}

---

When you want your hero to rifle through a desk and locate a specific document, scale a castle wall, negotiate a treaty with a monarch, or undertake any other activity with a chance of failure, you'll need to make a [test](../../Browse/rule/test/test.md) to determine how successful you are at the task. A [test](../../Browse/rule/test/test.md) is any [power roll](../../Browse/rule/dice/power-roll.md) that has failure or consequences as an option.

### When to Make a Test

The Director should ask a player to make a [test](../../Browse/rule/test/test.md) only when the player's hero attempts a task where the consequences of failure are interesting or dramatic, and where failure won't grind the story to a halt. For example, if a hero wants to leap over a waist-high wall while casually walking through a peaceful city neighborhood, the worst case for failure is probably that the hero falls on their butt, takes no damage, and can stand up to either try again or walk around the wall. As such, no [test](../../Browse/rule/test/test.md) is required. But if the hero were being chased by enemies, failing to leap over the wall means the pursuers can catch them, so the Director might decide to call for a [test](../../Browse/rule/test/test.md) to determine what happens.

The advancement of a story shouldn't be halted by failing a [test](../../Browse/rule/test/test.md). For instance, the heroes might need to know the color of a [dwarf](../../Browse/ancestry/dwarf.md) king's crown to solve a puzzle, with that puzzle opening the only entrance to a tomb they must enter to stop a world-ending ritual. It could be that a successful [Reason](../../Browse/rule/character/reason.md) [test](../../Browse/rule/test/test.md) allows the heroes to recall that lore, but the [test](../../Browse/rule/test/test.md) shouldn't be their only option to get the information. If the [test](../../Browse/rule/test/test.md) fails, perhaps the heroes need to go to a [flying](../../Browse/movement/fly.md) library to do research, or they might be able to delve into a ruin to find the ancient monarch's portrait. A failed [test](../../Browse/rule/test/test.md) should always result in a story becoming more interesting, not in the action coming to an end.

#### It Just Works!

When a hero attempts to solve a task that typically requires a [test](../../Browse/rule/test/test.md) with clever, outside-the-box thinking, the Director can instead decide that no [test](../../Browse/rule/test/test.md) is required and the attempt automatically works! For example, if a hero who wants to climb a wall first covers their hands and feet in giant strands of sticky spider webs, the Director might decide that they can climb up the wall without needing to make a [test](../../Browse/rule/test/test.md).
...
> **Influencing Player Characters With [Tests](../../Browse/rule/test/test.md)**
>
> The things player characters do can't be influenced by any creature making a [test](../../Browse/rule/test/test.md), whether a monster, an [NPC](../../Browse/rule/general/npc.md), or another player character. Many players feel that their agency is taken away if they're compelled to jump into a pile of gold filled with hidden scorpions because an [NPC](../../Browse/rule/general/npc.md) convinced them to do so with a [Presence](../../Browse/rule/character/presence.md) [test](../../Browse/rule/test/test.md). For most players, it's not fun to be in control of a hero and lose some of that control.
>
> Instead, Directors should do their level best to have an [NPC](../../Browse/rule/general/npc.md) suggest that a character dive headlong into the gold like a billionaire duck, then let the player decide what their character does. Similarly, a Director might decide that one player character can't make an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to discern another PC's motivations or honesty.
>
...
On a [test](../../Browse/rule/test/test.md)-by-[test](../../Browse/rule/test/test.md) basis, the Director can share the difficulty of a task before the player makes the [test](../../Browse/rule/test/test.md), which makes interpreting the outcome faster at the table. The Director can also keep a [test](../../Browse/rule/test/test.md)'s difficulty secret until after the player rolls the [test](../../Browse/rule/test/test.md), for dramatic effect.
...
For instance, if a hero suffers a consequence while trying to climb a wall, they might make it halfway up the wall and then fall, taking damage and landing [prone](../../Browse/condition/prone.md). A hero trying to sneak by cultists might be spotted by those foes, who immediately attack. If a consequence strikes when a hero attempts to bribe a prison guard, the guard might decide to arrest the hero or lead them into a trap. If a hero suffers a consequence on a [Reason](../../Browse/rule/character/reason.md) [test](../../Browse/rule/test/test.md) made to recall lore about the king's favorite meal, they might confuse it for a dish to which the monarch is deathly allergic.

Not all consequences need to be immediate or apparent. For example, a hero might fail with a consequence on a [test](../../Browse/rule/test/test.md) made to cheat at a high-stakes game of cards with a noble. The failure means the cheating is noticed, but the Director decides that the noble doesn't say anything. This consequence isn't made apparent until later in the evening, when the noble has guards surround the hero, intent on taking the cheater down to the dungeon for stacking the deck.

Common consequences for failing a [test](../../Browse/rule/test/test.md) include the following:

- Making an [NPC](../../Browse/rule/general/npc.md) so upset that they storm off, or betray, attack, or otherwise attempt to harm you
...
- Getting stuck in a situation that must be resolved with a negotiation or a [montage test](../../Browse/rule/test/montage-test.md) you didn't need to make before
...
The Director determines the reward for a success, which is most often related to the task at hand. For instance, if a hero succeeds with a reward while climbing a wall, they might find a ladder at the top they can lower so that any allies climbing up after them can do so without needing to make a [test](../../Browse/rule/test/test.md). A hero trying to sneak by cultists who succeeds with a reward might be able to dose the cultists' nearby water barrel with sleeping poison as they pass by unseen. Succeeding with a reward while bribing a prison guard could mean that the guard unlocks a door for the hero in addition to forgetting they were ever there.

As with consequences, the reward that comes with a success doesn't need to be immediate or apparent. For example, a hero succeeds with a reward on an easy [test](../../Browse/rule/test/test.md) made to cheat at a high-stakes game of cards with a noble. Not only does the hero win the game, but the Director decides that their reward comes from a servant watching the game who's impressed with the character's performance. After the game, the servant approaches the hero, offering magic from the noble's private stash in congratulations and admiration.

Common rewards accompanying success on a [test](../../Browse/rule/test/test.md) include the following:

- Automatically accomplishing a related follow-up task that would typically require a [test](../../Browse/rule/test/test.md)
- Allowing an ally engaged in the same task to accomplish the task without needing to make the [test](../../Browse/rule/test/test.md) as well
- Obtaining a [consumable](../../Browse/rule/treasure/consumable.md) treasure or useful piece of mundane equipment
...
In lieu of other rewards, the Director can also decide that a hero who succeeds on a [test](../../Browse/rule/test/test.md) with a reward earns the players a [hero token](../../Browse/rule/resource/hero-token.md) (see [Hero Tokens](../../Browse/rule/resource/hero-token.md) in Chapter 1: [The Basics](the-basics.md)).
...
The Director decides when the circumstances have changed enough to allow a new attempt at a [test](../../Browse/rule/test/test.md).

### Heroes Make Tests

If a hero attempts to sneak by an enemy guard unnoticed, should the hero make an [Agility](../../Browse/rule/character/agility.md) [test](../../Browse/rule/test/test.md) to sneak, or should the guard make an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to catch the hero in the act? If a cultist lies to a hero about the location of a secret temple, does the cultist roll a [Presence](../../Browse/rule/character/presence.md) [test](../../Browse/rule/test/test.md) to conceal the truth, or does the hero roll an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to discern the cultist's honesty?
...
At times, the Director might choose for an [NPC](../../Browse/rule/general/npc.md) to make a [test](../../Browse/rule/test/test.md) when engaged in a deceptive task, rather than having characters attempt to note the deception. By having the [NPC](../../Browse/rule/general/npc.md) roll in these scenarios, the Director doesn't tip their hand to the players that subterfuge is afoot.

For example, when an assassin attempts to ambush the heroes while they sit around a campfire, if any player says their hero is on the lookout for danger, that hero would make an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to notice the danger. But if no one is keeping watch, the assassin makes an [Agility](../../Browse/rule/character/agility.md) [test](../../Browse/rule/test/test.md) to sneak up on the heroes unnoticed. If the assassin fails the [test](../../Browse/rule/test/test.md), the heroes notice immediately as their assailant loudly steps on a twig. If the assassin succeeds, the heroes don't notice until the assailant is right on top of them.

An [NPC](../../Browse/rule/general/npc.md) might also make a [Presence](../../Browse/rule/character/presence.md) [test](../../Browse/rule/test/test.md) if they lie to the heroes, as long as the heroes have no reason to believe the character would be deceptive. The Director knows if the heroes are wary in that way because the players will ask if they can make a [test](../../Browse/rule/test/test.md) to discern the [NPC](../../Browse/rule/general/npc.md)'s honesty.
...
For example, if your hero attempts to sneak by a demon lord, you make an [Agility](../../Browse/rule/character/agility.md) [test](../../Browse/rule/test/test.md) to move stealthily while the demon makes an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to notice you. If your hero gets the higher [power roll](../../Browse/rule/dice/power-roll.md), you sneak by without the demon noticing. If the demon gets the higher roll, they catch you in the act of sneaking. If multiple sneaking heroes attempt to get by multiple demons, then each creature makes a [test](../../Browse/rule/test/test.md) and all the totals are compared to determine which demons notice which heroes.
...
Reactive [tests](../../Browse/rule/test/test.md) are typically made in the following circumstances, though the Director can call for them in any appropriate scenario:

**Hidden Environmental Features:** The Director asks for an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to notice a secret door, a hidden trap, or some other disguised environmental feature.

- **Hidden Foes:** The Director calls for an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) to notice a hidden foe (or they could use the rules in [NPCs](../../Browse/rule/general/npc.md) Roll for Deceptive Tasks above).
- **Hidden Motives:** The Director can ask for an [Intuition](../../Browse/rule/character/intuition.md) [test](../../Browse/rule/test/test.md) during a conversation to gauge how well a hero can read an [NPC](../../Browse/rule/general/npc.md), judging whether they're lying, withholding information, or concealing an emotional response. (The Director could also use the rules in [NPCs](../../Browse/rule/general/npc.md) Roll for Deceptive Tasks.)
- **Recall Lore:** The Director might ask a hero to make a [Reason](../../Browse/rule/character/reason.md) [test](../../Browse/rule/test/test.md) when dealing with a new object, a piece of information, or an event to determine what history or details the hero might already know about it.
- **Creature and DTO [Tests](../../Browse/rule/test/test.md):** Some creatures and dynamic terrain objects in *Draw Steel: Monsters* have features and abilities that require heroes to make reactive [tests](../../Browse/rule/test/test.md). These [tests](../../Browse/rule/test/test.md) can't be modified by skills.

> **Optional Rule: Secret Reactive [Tests](../../Browse/rule/test/test.md)**
>
> Some Directors prefer to make the [power rolls](../../Browse/rule/dice/power-roll.md) for reactive [tests](../../Browse/rule/test/test.md) for the heroes rather than asking the players to do so. This allows the Director to make the rolls when appropriate for hidden objects, creatures, motivations, and information without tipping off the players that there is information to be gained. Having the Director roll requires the Director to have everyone's [characteristics](../../Browse/rule/character/characteristic.md) and skills recorded (whether physically or digitally) for easy reference.
...
**Director:** *Okay. It'll be a [Might](../../Browse/rule/character/might.md) [test](../../Browse/rule/test/test.md) to get up to the top of the wall.*
...
The Director checks the [Test Difficulty](../../Browse/rule/test/test-difficulty.md) Outcomes table to confirm that a tier 3 outcome is a success on a hard [test](../../Browse/rule/test/test.md).
...
**Grace:** *So it's a [Might](../../Browse/rule/character/might.md) [test](../../Browse/rule/test/test.md). Would you let me use Endurance? It's a lot of physical exertion to climb.*

**Director:** *It's not a climb that would take you hours, so I don't think Endurance applies here. But hey, the rope does make this an easier [test](../../Browse/rule/test/test.md).*

**Grace:** *Fair. Okay, dice. Let's do this.*

Given the rope, the Director decides that this second climb attempt is an easy [test](../../Browse/rule/test/test.md). Grace rolls 2d10 and gets an 11. With her [Might](../../Browse/rule/character/might.md) score of 2, that's a 13.

**Grace:** *13! That's a tier 2 outcome for Val.*

The Director checks the [Test Difficulty](../../Browse/rule/test/test-difficulty.md) Outcomes table once more, confirming that a tier 2 outcome is a success on an easy [test](../../Browse/rule/test/test.md).
...
**Director:** *Sure. That'll be a [Might](../../Browse/rule/character/might.md) [test](../../Browse/rule/test/test.md) if Korvo's down for it.*
...
If a creature is hidden from your allies but not from you, you can use a maneuver without making a [test](../../Browse/rule/test/test.md) to point that creature out to your allies.
...
For larger or smaller groups, the Director can make the following adjustments to keep [montage tests](../../Browse/rule/test/montage-test.md) achievable but challenging:
...
- If the heroes hit the success limit before hitting the failure limit or before the time runs out for the [test](../../Browse/rule/test/test.md), they achieve total success.
